...
func prependLanguage(child: Block) -> [Block] {
  ...
  let explanation = Block.Paragraph(
    text: [.Text(text: "Example: "), .Emphasis(children: [.Text(text: language!)]) ])
...
func stripSampleImpl(child: Block) -> [Block] {
  guard case let .CodeBlock(code, language) =
    child where language == "swift" else { return [child] }
...
func tableOfContents(blocks: [Block]) -> [Block] {
  ...
  return [Block.Paragraph(text: [InlineElement.Emphasis(children: ["Table of contents"])])]
    + headers
...
class RenderViewController: NSViewController {
  ...
  func loadNode(fileName: String)(elements: [Block]) {
    ...
    let restoreScrollTop = "<script type='text/javascript'>"
      + "document.body.scrollTop = \(scrollTop)"
      + "</script>"
    let directory    = (fileName as NSString).stringByDeletingLastPathComponent
    let directoryURL = NSURL(fileURLWithPath: directory)
    let document     = tableOfContents(elements)
      + deepApply(elements, { stripSampleImpl($0).flatMap(replaceOrError(directory)) })
    let elements = evaluateAndReplacePrintSwift(document, workingDirectory: directoryURL)
  ...
  override func viewDidAppear() {
    if let
      doc = view.window?.windowController?.document as? MarkdownDocument,
      fileName = doc.fileURL?.path
    {
